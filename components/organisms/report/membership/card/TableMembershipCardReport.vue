  ...
  methods: {
    ...
    async processGetData() {
      ...
      try {
        ...
        this.$sentry.captureMessage(`${error.message} at processGetData in TableMembershipCardReport`)
      }
...
<template>
  <div>
    <content-table-view :length="data.rows.length" :is_loading="helper.is_loading" :is_error="helper.is_error"
      @reload="processGetData" loading-caption="Laporan belum tersedia" test_id="wrapper__table-membership-card-report"
      ref="wrapper__table-membership-card-report">
      <div class="table-responsive rounded-top mb-0">
        <table class="d-block table table-striped table-sticky-header report-table"
          data-testid="table-membership-card-report">
          <thead class="thead-primary">
            <tr>
              ...
              <th class="cell-sticky" style="top: 1px" data-testid="th-identity">
                Identitas
              </th>
              <th class="cell-sticky" style="top: 1px" data-testid="th-rfid">
                RF ID
              </th>
              <th class="cell-sticky" style="top: 1px" data-testid="th-plate-1">
                Plat Nomor 1
              </th>
              <th class="cell-sticky" style="top: 1px" data-testid="th-plate-2">
                Plat Nomor 2
              </th>
              <th class="cell-sticky" style="top: 1px" data-testid="th-card-number">
                Nomor Kartu
              </th>
          ...
          <tbody>
            <tr v-for="(row, index) in data.rows" :key="index">
              ...
              <td :data-testid="`tr-${index}__td-identity`">
                {{ row.identification_number }}
              </td>
              <td :data-testid="`tr-${index}__td-rfid`">{{ row.rf_id }}</td>
              <td :data-testid="`tr-${index}__td-plate-1`">
                <plain-label :ref="`tr-${index}__label-plate-1`" :test_id="`tr-${index}__label-plate-1`"
                  :value="row.license_plate" />
              </td>
              <td :data-testid="`tr-${index}__td-plate-2`">
                <plain-label :ref="`tr-${index}__label-plate-2`" :test_id="`tr-${index}__label-plate-2`"
                  :value="row.second_license_plate" />
              </td>
              <td :data-testid="`tr-${index}__td-card-number`">
                {{ row.printed_id }}
              </td>
          ...
          <tfoot>
            <tr>
              ...
              <td v-for="empty in 16" :key="empty">-</td>
              <td>
                <price-label :test_id="`total__label-amount`" :has_label="true"
                  :value="data.total ? data.total.amount : 0" />
              </td>
            </tr>
          </tfoot>
        </table>

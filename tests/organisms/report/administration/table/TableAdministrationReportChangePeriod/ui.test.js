...
describe("ui.TableAdministrationReportChangePeriod.vue", () => {
  let mocksFn = {}
  ...
  it("table should display data according report data", async () => {
    ...
    for (let index = 0; index < list_tr.length; index++) {
      ...
      expect(list_td.at(2).text()).toContain(reportContent[index].previous_end_date)
      expect(list_td.at(3).text()).toContain(reportContent[index].updated_end_date)
      expect(list_td.at(4).text()).toContain(reportContent[index].user)
    }
  })
})
...
import TableMembershipPlateReport from "@/components/organisms/report/membership/plate/TableMembershipPlateReport"
...
describe("unit.TableMembershipPlateReport.vue", () => {
  beforeEach(() => {
    mocksFn = {}
    mocksFn['processGetData'] = jest.spyOn(TableMembershipPlateReport.methods, "processGetData").mockImplementation()
  })
  ...
  describe("mounted lifecycle", () => {
    test.each([
      ...
      wrapper = shallowMount(TableMembershipPlateReport, { localVue, mocks, propsData })
  ...
  describe("method", () => {
    beforeEach(() => {
      wrapper = shallowMount(TableMembershipPlateReport, { localVue, mocks })
    })
    ...
    describe("processGetData", () => {
      it("given no error occured, should set data with returned value from getMembershipReportPlateByDate", async () => {
        wrapper.vm.setPayloadGet = mocksFn['setPayloadGet'] = jest.fn().mockReturnValueOnce(payloadGetBasic)
        wrapper.vm.getMembershipReportPlateByDate = mocksFn['getMembershipReportPlateByDate'] = jest.fn().mockReturnValueOnce(dataGet)
        ...
        wrapper.vm.processGetData = TableMembershipPlateReport.methods.processGetData
        await wrapper.vm.processGetData()

        expect(mocksFn['setPayloadGet'].mock.calls).toEqual([[]])
        expect(mocksFn['getMembershipReportPlateByDate'].mock.calls).toEqual([[payloadGetBasic]])
      ...
      it("given error occured, should set helper.is_error to true", async () => {
        ...
        wrapper.vm.getMembershipReportPlateByDate = mocksFn['getMembershipReportPlateByDate'] = jest.fn().mockRejectedValueOnce(error)
        wrapper.vm.passFormatDownloadReport = mocksFn['passFormatDownloadReport'] = jest.fn()
        wrapper.vm.$utility.setErrorContextSentry = mocksFn['$utility.setErrorContextSentry'] = jest.fn()
        wrapper.vm.$sentry.captureMessage = mocksFn['$sentry.captureMessage'] = jest.fn()

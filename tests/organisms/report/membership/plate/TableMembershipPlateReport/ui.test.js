...
describe("ui.TableMembershipPlateReport.vue", () => {
  beforeEach(async () => {
    mocksFn = {};
    mocksFn["processGetData"] = jest
      .spyOn(TableMembershipPlateReport.methods, "processGetData")
      .mockImplementation();
  ...
  test("given filled data.rows, should show each row on table", async () => {
    for (let indexRow = 0; indexRow < dataGet.rows.length; indexRow++) {
      ...
      element = wrapper.findComponent({ ref: `tr-${indexRow}__label-plate` });
      expect(element.exists()).toBeTruthy();

...
try {
  ...
  window.$nuxt.$sentry.captureMessage(
    `${error.message} at rendering filter-report in report/memebership/plate/index`
  );
...
export default {
  head: () => ({
    title: `Laporan Membership - Membership Plat | ${process.env.APP_TITLE}`
  }),
  components: {
    ...
    TableMembershipPlateReport: () => import("@/components/organisms/report/membership/plate/TableMembershipPlateReport"),
  },
  data: () => ({
    filter: {},
    helper: {
    ...
    report: {
      name: "",
      fields: constant_report.membership_plate_report(),
      content: []
    },
    options: {
  ...
  methods: {
    ...
    passExportType(value) {
      try {
        switch (value) {
          case 'PDF':
            window.open(`/report/membership/plate/print?month=${this.filter.month}&user=${this.filter.user.value ?? ""}&product=${this.filter.product}&spot=${this.$utility.getSpotId()}`, "_blank")
            return
          case 'CSV':
        ...
        this.$sentry.captureMessage(`${error.message} at passExportType in report/membership/plate`)
      }
    },
    ...
    <square-card :has_shadow="true">
      <b-row class="px-2">
        <b-col cols="12" lg="6" class="px-1">
          <content-title additional_class="mb-0 px-0 mx-0 pt-2" value="Laporan Membership Plat"/>
        </b-col>
        ...
        <b-col cols="12" class="px-0">
          <table-membership-plate-report :month="filter.month" :product="filter.product" :user="filter.user"
                                         :is_searching="helper.is_searching" @ready="processPayloadDownloadReport"
                                         ref="table_membership_plate_report"/>
        </b-col>
      </b-row>
    </square-card>
  </layout>

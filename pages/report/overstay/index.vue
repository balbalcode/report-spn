...
try {
  ...
  window.$nuxt.$sentry.captureMessage(
    `${error.message} at rendering filter-report in report/overstay/index`
  );
  ...
  methods: {
    ...
    async processGetReport(payload) {
      if (this.data_type.value === "income")
        return await this.getIncomeOverstayReport(payload);
      else if (this.data_type.value === "quantity")
        return await this.getQuantityOverstayReport(payload);
    },
    ...
    processPayloadDownloadReport(value) {
      ...
      this.report.name = `Laporan ${this.data_type.text} Parkir Inap ${spot.text} Bulan ${this.filter.month}`;
    ...
    <square-card :has_shadow="true">
      <div class="row px-2">
        <div class="col-12 col-lg-6 px-1">
          <content-title additional_class="mb-0 px-0 mx-0 pt-2" value="Parkir Inap" />
        </div>
        <div class="col-12 col-lg-6 text-right">
          ...
          <export-button :export_name="report.name" :fields="report.fields" :content="report.content" id="btn-export"
            :is_disabled="report.content.length < 1" additional_class="mt-2" style="display: none" test_id="btn-export" />
        </div>
      </div>
      <div>
        <content-divider />
      ...
      <div class="d-flex align-items-center flex-wrap mb-3">
        <filter-report has_vehicle has_period has_spot has_column_setting has_type is_init_all_column is_init_all_vehicle
          is_not_update_init has_timezone :option_column="options.column" @update="processGetPayloadFilter" id="payment"
      ...
      <div>
        <table-report-by-payment :month="filter.month" :vehicle="filter.vehicle" :column="filter.column"

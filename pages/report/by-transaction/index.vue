<script>
import { download_types, transaction_types, report_data_types } from "~/constants/v3.report";
...
try {
  ...
  window.$nuxt.$sentry.captureMessage(
    `${error.message} at rendering filter-report in report/by-transaction/index`
  );
...
export default {
  components: {
    ...
    FilterReport,
    TableReportByTransaction: () => import("@/components/organisms/report/by-transaction/TableReportByTransaction"),
  ...
  async created() {
    ...
    this.transaction_types = this.options.column = transaction_types(this.data_type.value);
  },
  methods: {
    ...v3reportMethods,
    ...
    passExportType(value) {
      try {
        switch (value) {
          case "PDF":
            window.open(
              `/report/v3/${this.data_type.value}/transaction/print?month=${this.filter.month
        ...
        this.$sentry.captureMessage(
          `${error.message} at passExportType in ReportByTransaction`
        );
      }
    },
    ...
    async processGetReport(payload) {
      if (this.data_type.value === "income")
        return await this.getIncomeReportByTransaction(payload);
      else if (this.data_type.value === "quantity")
        return await this.getQuantityReportByTransaction(payload);
    },
    ...
    processPayloadDownloadReport(value) {
      ...
      this.report.name = `Laporan ${this.data_type.text} Jenis Transaksi ${spot.text} Bulan ${this.filter.month}`;
...
<template>
  <layout :title="`Laporan ${data_type.text}`">
    <square-card :has_shadow="true">
      <div class="row px-2">
        <div lg="6" class="col-12 col-lg-6 px-1">
          <content-title additional_class="mb-0 px-0 mx-0 pt-2" value="Jenis Transaksi" />
        </div>
        <div lg="6" class="col-12 col-lg-6 text-right">
      ...
      <div class="d-flex align-items-center flex-wrap mb-3">
        <filter-report has_vehicle has_period has_column_setting is_init_all_column is_init_all_vehicle id="transaction"
          has_type has_timezone :option_column="options.column" @update="processGetPayloadFilter" ref="filter-report"
      ...
      <div>
        <table-report-by-transaction :month="filter.month" :vehicle="filter.vehicle" :column="filter.column"
          ...
          :get_report="processGetReport" :transaction_types="transaction_types" />
      </div>
    </square-card>
    <!-- modal warning -->

...
try {
  ...
  window.$nuxt.$sentry.captureMessage(
    `${error.message} at rendering filter-report in report/additional/index`
  );
...
export default {
  head: () => ({
    title: `Laporan Keuangan - Laporan Layanan Tambahan | ${process.env.APP_TITLE}`
  }),
  components: {
    ...
    TableServiceQuantityReport: () => import("@/components/organisms/report/additional/TableServiceQuantityReport"),
  },
  data: () => ({
    filter: {},
    helper: {
  ...
  methods: {
    passExportType(value) {
      try {
        switch (value) {
          case 'PDF':
            window.open(`/report/additional/print?month=${this.filter.month}&service=${this.formatServiceList()}&spot=${this.$utility.getSpotId()}`, "_blank")
            return
          case 'CSV':
        ...
        this.$sentry.captureMessage(`${error.message} at passExportType in report/additional`)
      }
    },
    ...
    processPayloadDownloadReport(value) {
      ...
      this.report.name = `Laporan Layanan Tambahan ${spot.text} Bulan ${this.filter.month}`
    ...
    <square-card :has_shadow="true">
      <b-row class="px-2">
        <b-col cols="12" lg="6" class="px-1">
          <content-title additional_class="mb-0 px-0 mx-0 pt-2" value="Laporan Layanan Tambahan"/>
        </b-col>
        ...
        <b-col cols="12" class="px-1 mb-3">
          <filter-report :has_period="true" :has_spot="true" :has_service="true" :is_disabled="helper.is_searching"
            @update="processGetPayloadFilter"/>
        </b-col>
        <b-col cols="12" class="px-0">
          <table-service-quantity-report :month="filter.month" :service="filter.service"
                                         :is_searching="helper.is_searching" @ready="processPayloadDownloadReport"
                                         ref="table_service_quantity_report"/>
        </b-col>
      </b-row>
    </square-card>
  </layout>
